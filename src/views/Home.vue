<template>
    <div class="container flex flex-col gap-y-5 max-w-7xl my-0 mx-auto py-10 px-5">
      ...
      <div class="control-panel flex">
        <button class="p-[5px] rounded-[10px] shadow-[0_0_2px_1px_rgba(100,100,100,.7)] transition linear duration-300 cursor-pointer dark:text-white" @click="switchTheme()">
          <span v-if="theme">Dark Mode</span>
          <span v-else>Light Mode</span>
        </button>
      </div>
    <ul class="grid grid-cols-grid-list-column gap-10">
        <li class="rounded-[10px] shadow-[0_0_2px_1px_rgba(150,150,150,.8)] transition linear duration-300 cursor-pointer overflow-hidden hover:scale-105" v-for="country in getterData" :key="country.name">
          <router-link :to="{
            name: 'countryPage',
            params: {
              name: country.name,
